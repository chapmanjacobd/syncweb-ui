...
func (s *Syncweb) Init(params map[string]string, app *App) (IBackend, error) {
	...
	if backend.APIKey == "" || strings.Contains(backend.URL, "127.0.0.1:8384") {
		xmlConfig, err := s.loadConfigFromXML("syncweb")
		if err == nil {
...
func (s Syncweb) Ls(path string) ([]os.FileInfo, error) {
	...
	for _, stFile := range stFiles {
		...
		files = append(files, File{
			...
			FType: fType,
			FTime: stFile.ModTime / 1000, // convert Syncthing nanoseconds to Filestash microseconds
			FSize: stFile.Size,
		})
	}
...
func (s Syncweb) Rm(path string) error {
	...
	// File exists locally, perform deletion
	if err := os.Remove(localPath); err != nil {
		return NewError("Failed to remove local file: "+err.Error(), 500)
	}
	return nil
}
...
func (s Syncweb) Touch(path string) error {
	return s.Save(path, strings.NewReader(""))
}

func (s Syncweb) Save(path string, file io.Reader) error {
	localPath, err := s.resolveLocalPath(path)
	if err != nil {
	...
	if os.IsNotExist(statErr) {
		return NewError("Parent directory not found locally. Cannot create remote Syncthing files.", 405)
	} else if statErr != nil {
		return NewError("Filesystem access error: "+statErr.Error(), 500)
	}
	...
	if err != nil {
		return NewError("Failed to read file content: "+err.Error(), 500)
	}

	if err := os.WriteFile(localPath, content, 0o644); err != nil {
		return NewError("Failed to save local file: "+err.Error(), 500)
	}
...
func (s Syncweb) getSyncwebConfigPath(appName string) string {
	if home == "." {
...
func (s Syncweb) loadConfigFromXML(appName string) (*SyncthingConfigXML, error) {
	configPath := s.getSyncwebConfigPath(appName)
	...
	if err := xml.Unmarshal(data, &stConfig); err != nil {
		return nil, fmt.Errorf("failed to parse XML config from %s: %w", configPath, err)
	}
...
func (s Syncweb) syncwebDefaultFolderRoot() string {
	...
	if err == nil {
		if root, ok := def["path"].(string); ok && root != "" {
			return filepath.Clean(root)
		}
	}

	// fallback: $HOME/Syncweb
...
func (s Syncweb) Mkdir(path string) error {
	...
	if parent == "/" {
		...
		if _, err := os.Lstat(path); err == nil {
			return NewError(fmt.Errorf("%s: %w", path, err).Error(), 400)
		}
		// TODO: investigate what is FPath; it may be worth trying to use the folderID for FPath and the basename for FName
